...
def bronze_patient_visits():
    ...
    columns = ["visit_id","patient_name","city","department",
               "consultation_fee","number_of_tests"]
...
def silver_patient_visits():
    ...
    return df.withColumn("department", upper(col("department"))) \
             ...
             .select("visit_id", "patient_name", "city", "department",
                     "consultation_fee", "number_of_tests", "test_cost", "total_bill")
             
@dlt.table
def gold_hospital_summary():
    df = dlt.read("silver_patient_visits")
    return df.groupBy("city", "department") \
             .agg(
